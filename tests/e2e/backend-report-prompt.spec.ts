...
test.describe.serial('AI Prompt Generation & Jargon-Free E2E', () => {
  ...
  test('연애(love) 테마 프롬프트 분기 및 검증', async ({ page }) => {
    ...
    expect(logData.systemPrompt).toContain('조언 후처리');
    expect(logData.systemPrompt).toContain('성별 본질론');
    expect(logData.systemPrompt).toContain('정복');
    expect(logData.systemPrompt).toContain('소유');
    expect(logData.systemPrompt).toContain('밀당');
    expect(logData.systemPrompt).toContain('조종');
    expect(logData.systemPrompt).toContain('최종 리포트에 rule_id, source_book, source_excerpt를 노출하지 않는다');
    ...
    expect(logData.userContext).toContain('[TRAIT_FINDINGS]');
    expect(logData.userContext).toContain('[RISK_PATTERNS]');
