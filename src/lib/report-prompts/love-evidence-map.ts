...
const LOVE_SECTION_EVIDENCE: LoveSectionEvidence[] = [
  ...
  },
  {
    ...
    writingInstruction: "어떤 매력이 잠재되어 있는가, 이성이 어떤 매력을 느끼는가, 성적 끌림을 하게 하는 나의 매력 포인트, 매력이 약해지는 순간, 매력을 기르는 방법 순서로 쓴다. 남성 리포트는 여성이 실제로 체감하는 매력 장면으로, 여성 리포트는 남성이 실제로 체감하는 매력 장면으로 번역한다. 성별 정체성과 충돌하는 외모 수식어는 쓰지 않는다. 성적 끌림 포인트는 [CHARM_ACTION_RULES]의 상대 이성이 체감하는 장면과 성별별 어필 장면을 우선 사용하고, 신체 단서는 보조 근거로만 사용한다. 우아함, 기품, 자기장, 아우라, 도화, 반전 매력 같은 추상어는 최종 출력에서 단독으로 쓰지 말고, 상대 이성이 실제로 체감하는 말투, 표정, 속도, 거리감, 대화 후 기억에 남는 장면으로 번역한다. 성적 끌림은 정서적 안정감으로 대체하지 말고 시선, 거리감, 몸의 긴장감, 가까워지고 싶음으로 구체화한다. 관록궁/재백궁은 이 섹션 핵심 근거로 쓰지 않는다. 천이궁은 외부 반응 보조만 사용한다.",
  },
  {
