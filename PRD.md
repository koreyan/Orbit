...
| 리포트 보관함/상세/공유 | 유지+UX 정리 | `/reports`, `/reports/[report-id]`, 링크 복사 공유 Route Handler 유지. 소유자 상세에는 명반 표를 초기 펼침으로 제공하고, 공개 방문자에게 명반 데이터/UI를 제공하지 않음. Web Share 버튼은 제거 |
...
| URL | 역할 | 수용 기준 |
|---|---|---|
| `/` | 랜딩/명반 입력 | 유효 입력 시 명반 데이터 생성, 누락/무효 입력 차단. 실제 저장 정책과 충돌하는 “생년월일시 미저장” 안내는 표시하지 않음 |
| `/result` | 명반 및 테마 선택 | 새로고침 가능한 쿼리 기반 명반 표시, 대한/유년 타임라인·가이드·테마 선택 유지, 테마 미선택 주문 차단 |
| `/checkout` | 주문 확인/결제 | `orderId`로 최소 주문 필드 조회. 0원 주문은 안내 카드 없이 결제 상세와 `0원으로 시작하기` CTA만 중앙 표시, 유료 주문만 Toss 위젯 표시 |
...
| `/reports/[report-id]` | 리포트 상세 | 본인 또는 공개 공유 리포트만 조회. 본인 상세에는 명반 표만 초기 펼침 상태로 표시하고 `명반 접기`/`내 명반 보기` 토글 제공. 공개 방문자에게 명반 원본·차트 데이터·명반 UI를 제공하지 않음. 링크 복사는 유지하고 Web Share는 제공하지 않음. 연애 리포트에는 `5-2. 연애 팁` 섹션을 생성·저장하지 않음 |
...
- 명반 grid는 `/result`와 소유자 리포트 상세에서 공통 표시 컴포넌트로 재사용한다.
- `/reports/[report-id]`의 명반 접기/펼치기는 `aria-expanded`와 `aria-controls`를 상태에 맞게 제공한다.
- 0원 checkout은 단일 결제 상세 카드와 실제 CTA 중심으로 표시한다.
...
| `src/app/actions/myeongban.ts` | `getMyeongbanAction` | 공통 `getMyeongban` 서버 서비스의 thin wrapper. 입력 생년월일시로 명반 생성 및 기초 해석 조회 |
| `src/app/actions/order.ts` | `createAnonymousOrderAction`, `getOrderAction`, `linkUserToOrderAction` | 주문 생성은 서버 상품표 기준 금액 사용, 주문 생성 시 claim token을 HttpOnly 쿠키로 발급하고 DB에는 hash/만료/사용 시각만 저장 |
| `src/app/actions/payment.ts` | `confirmPaymentAction` | 무료 결제는 활성 `love` 0원 데모 주문만 허용, mock/E2E 결제키는 비운영에서만 허용 |
| `src/app/actions/report.ts` | `generateReportAction`, 공유 관련 액션 | 리포트 생성/상태 전이/OpenAI 호출/알림 오케스트레이션. 연애 리포트는 `5-2. 연애 팁`과 `directionGuide` 입력 없이 생성하고, 저장 전 후처리로 해당 섹션을 방어적으로 제거. 리포트 생성 파이프라인 전체 실패는 `failed` 상태와 Telegram 실패 알림으로 전이하며, 최종 DB 저장이 확인된 뒤에만 성공 알림을 발송한다 |
...
- 결제 완료 주문의 계정 귀속은 주문 UUID와 HttpOnly claim cookie를 함께 검증하고, 성공 즉시 token 사용 시각을 기록해 재사용을 막는다.
...
- 공개 링크 방문자에게 주문의 `saju_data`, 재계산 명반, 명반 UI를 전달하지 않는다.
- 공유는 링크 복사 기반 공개 설정만 유지하며 브라우저 Web Share API 버튼/호출은 제공하지 않는다.
...
| `orders` | 익명/귀속 주문, 사주 데이터, 테마, 금액, 상태, claim token hash/만료/사용 시각 | 결제 전 `pending`, 결제 후 `paid`, 계정 귀속 시 claim token 소모 |
...
2026-07-17 리포트 UX 개선에서는 schema 변경 없이 `reports.content.markdown` 내부의 기존 연애 리포트 `5-2. 연애 팁` 섹션만 백업 후 제거했다. dry-run 대상은 14건, apply 성공은 14건/실패 0건, apply 후 재실행 dry-run 잔존 대상은 0건이다. 백업 파일은 `data/backups/love-tip-removal-2026-07-17T03-33-50-035Z.json`이며 SHA-256은 `27e12f1a73bf9c6fceaf1fa45b12e1d9fc14ff537ce26bc7152787829a169794`이다.
...
- 리포트 생성 실패 알림은 OpenAI 호출 오류뿐 아니라 명반 추출 데이터 누락, 컨텍스트 생성 오류, DB 저장 오류 등 생성 파이프라인 전체 실패에 대해 Telegram으로 발송한다.
- 리포트 생성 성공 알림은 `reports.content`, `status: completed`, `generated_at` 저장이 실제로 성공하고 update row가 존재할 때만 발송한다.
- 리포트 재생성 성공 후 `/reports`와 `/reports/[report-id]`를 함께 revalidate해 실패 화면이 stale 상태로 남지 않도록 한다.
...
| `npm run test:unit` | 통과 | Vitest 9 files / 31 tests |
| `npm run build` | 통과 | Next build에서 TypeScript 실행, 17 static pages 생성 |
| `npm run verify` | 통과 | lint + typecheck + unit test + build |
| Playwright E2E | 부분 통과/환경 미완료 | `landing-page`, `checkout-page`, `result-page` desktop-chromium 11 tests 통과. 지정 전체 E2E는 `.env.test.local` 테스트 Supabase 값 부재로 backend specs가 import 단계에서 중단. WebKit 프로젝트는 Playwright WebKit browser 미설치로 실행 불가. `reports-page` desktop은 기존 mock order/session 전제와 현재 auth/DB 흐름 불일치로 실패 |
...
| 2026-07-17 | 서버 상품표, 무료 데모 결제 정책, claim token 주문 소유 증명, Vitest 단위 테스트 기준 추가 |
| 2026-07-17 | 리포트 UX 개선, 연애 팁 제거, owner-only 명반 상세, Web Share 제거, 리포트 생성 실패 알림/재생성 저장 검증 보강 |
