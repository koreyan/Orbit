...
| `src/app/actions/report.ts` | `generateReportAction`, 공유 관련 액션 | 리포트 생성/상태 전이/OpenAI 호출/알림 오케스트레이션. 연애 리포트는 `5-2. 연애 팁`과 `directionGuide` 입력 없이 생성하고, 저장 전 후처리로 해당 섹션을 방어적으로 제거 |
...
| `npm run test:unit` | 통과 | Vitest 8 files / 28 tests |
