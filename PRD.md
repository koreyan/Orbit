...
| `npm run test:unit` | 통과 | Vitest 5 files / 16 tests |
| `npm run build` | 통과 | Next build에서 TypeScript 실행 |
| `npm run verify` | 통과 | lint + typecheck + unit test + build |
| Playwright E2E | 미실행 | 테스트 Supabase 확인 전 DB 쓰기 E2E 금지. config는 `.env.test.local`과 `E2E_SUPABASE_PROJECT_REF`를 요구함 |
